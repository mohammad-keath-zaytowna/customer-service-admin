...
export async function POST(request: Request) {
  ...
  const { errors } = validateFormData(passwordUpdateFormSchema, { password, confirmPassword });
  if (errors) {
    return NextResponse.json({ errors }, { status: 401 });
  }

  try {
    const backendRes = await fetch(`${process.env.BACKEND_URL || 'http://localhost:5000'}/auth/update-password`, {
      method: 'POST',
    ...
    if (!backendRes.ok) {
      return NextResponse.json({ errors: payload?.errors ?? { password: payload?.message ?? 'Failed' } }, { status: 401 });
    }
